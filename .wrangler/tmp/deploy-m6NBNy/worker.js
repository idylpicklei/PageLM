...
async function loadFlashcards(db, userId) {
  const row = await db.prepare("SELECT value FROM kv WHERE key = ?").bind(flashcardsKey(userId)).first();
  if (!row?.value) return [];
  try {
    const parsed = JSON.parse(row.value);
    return Array.isArray(parsed) ? parsed : [];
...
async function handleGroupRoutes(request, env, pathname) {
  ...
  if (parts[1] === "items" && parts.length === 2 && request.method === "POST") {
    ...
    return json5({ error: "kind must be skill, file, or note" }, { status: 400 });
  }
...
var SPA_GET_PATHS = /* @__PURE__ */ new Set(["/chat", "/quiz", "/planner", "/debate", "/exam", "/login", "/signup", "/canvas", "/groups", "/groups/join"]);
...
var PageLMContainer = class extends Container {
  ...
  async fetch(request) {
    ...
    const running = Boolean(this.container?.running);
    if (!running) {
      try {
...
async function fetchContainer(container, request) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    if (attempt < 4) await scheduler.wait(2e3 * (attempt + 1));
  }
